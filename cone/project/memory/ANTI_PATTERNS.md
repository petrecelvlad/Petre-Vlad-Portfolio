...
### A `--` inside a hand-written XML `<!-- comment -->` silently breaks the whole SVG file
**What happened:** Across all 3 hand-authored SVG conversions in one session (Level-Up, Teams Lead, Cartridge — T-026), writing a descriptive top-of-file or inline comment containing a bare `--` (e.g. "reads more 'chunky rounded shape' than 'organic circular blob,' -- not a defect", "continuous 360 rotation -- not reproducible") broke XML well-formedness every single time, caught only by explicitly re-running the `python3 -m xml.dom.minidom` parse check after each edit.
**Root cause:** XML forbids `--` anywhere inside a comment's content, not just as the closing delimiter — a rule inherited from SGML that has nothing to do with normal prose conventions, where using `--` as an em-dash-ish separator is completely idiomatic English. Nothing about the resulting error (`not well-formed (invalid token)` at some line/column) mentions comments or double-hyphens directly, so it reads like a generic syntax problem rather than pointing at the actual cause.
**The fix:** When hand-writing prose inside an SVG/XML `<!-- -->` comment, use a single `;`, `,`, or em-dash character (`—`) instead of `--` for a parenthetical break. Re-validate with `python3 -c "import xml.dom.minidom as m; m.parse('file.svg')"` after every edit that touches a comment, not just after the initial write — this bug reliably slips back in on follow-up edits that add new explanatory comments, not just the first draft.
